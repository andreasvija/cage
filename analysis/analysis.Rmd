...
new_transcript_genes = readRDS("txrevise/scripts/processed/Homo_sapiens.GRCh38.96_CAGE-20/new_transcript_genes_20.rds")
length(new_transcript_genes) # 11 877
length(intersect(new_transcript_genes, common_genes)) # 11 349
remove(new_transcripts)
remove(new_transcript_genes)
```
...
remove(new_transcripts)
remove(trial_results)
remove(trial_results_common)
...
ggplot(metrics, aes(N, net_increase)) +
  geom_line() +
  geom_point() +
  scale_x_continuous(breaks=Ns) +
  labs(x='N', y='', title='How many more significant genes in total were found after promoter augmentation?')

ggplot(metrics, aes(N, removed_genes)) +
  geom_line() +
  geom_point() +
  scale_x_continuous(breaks=Ns) +
  labs(x='N', y='', title='How many genes lost significance after promoter augmentation?')

ggplot(metrics, aes(N, frac_added_in_bwa_tops)) +
  geom_line() +
  geom_point() +
  scale_x_continuous(breaks=Ns) +
  labs(x='N', y='', title='What fraction of significant genes were also significant when unaugmented?')
```
...
interesting_gene_bwa = -log10(filter(bwa_results, gene=="ENSG00000151694")["p_adjusted_beta_adjusted"][[1]])
interesting_gene_tx = -log10(filter(txrevise25_results, gene=="ENSG00000151694")["p_adjusted_beta_adjusted"][[1]])
interesting_gene_ann = -log10(filter(annots_results, gene=="ENSG00000151694")["p_adjusted_beta_adjusted"][[1]])
...
fig1 = ggplot(comparison, aes(x=-log10(tx_p), y=-log10(bwa_p))) +
  labs(x="txrevise -log10(p)", y="CAGE -log10(p)") +
...
comparison$grouping = factor(sapply(comparison$gene, getgrouping), 
                             levels=c("No change", "Became significant", "Lost significance"), 
                             ordered=TRUE)
comparison = arrange(comparison, grouping)

fig2 = ggplot(comparison, aes(x=-log10(ann_p), y=-log10(bwa_p), color=grouping)) +
  labs(x="Augmented txrevise -log10(p)", y="CAGE -log10(p)", color="Significance") +
...
print(fig1 +
        ...
        geom_point(alpha=0.8, color="dark gray") +
        geom_point(aes(x=interesting_gene_tx, y=interesting_gene_bwa), color="blue", size=2.5) +
        xlim(0,50) + ylim(0,50))
print(fig2 +
        geom_segment(x=0, y=0, xend=120, yend=120, color="black") +
        geom_point(aes(color=grouping), alpha=0.8) +
        scale_color_manual(values=c("dark gray", "green", "red")) +
        geom_point(aes(x=interesting_gene_ann, y=interesting_gene_bwa), color="blue", size=2.5) +
        xlim(0,50) + ylim(0,50) +
...
```{r}
phenotypes = c(filter(bwa_results, gene == "ENSG00000151694")$top_phenotype[1],
               filter(txrevise25_results, gene == "ENSG00000151694")$top_phenotype[1],
               filter(annots_results, gene == "ENSG00000151694")$top_phenotype[1])
cage_variant = filter(bwa_results, gene == "ENSG00000151694")$top_variant[1]
...
counts_bwa = read_tsv("../qtlmap_prep/countMatrix.tsv") %>%
  ...
  filter(phenotype_id %in% phenotypes)
#/gpfs/space/home/andreasv/cage/pipeline/txrevise/qcnorm_out/txrevise/normalised/txrev/qtl_group_split_norm/GEUVADIS.LCL.tsv
counts_tx = read_tsv("countMatrix_txrevise.tsv") %>%
  melt(id.vars=c("phenotype_id"), variable.name=c("sample_id"), value.name="proportion") %>%
  filter(phenotype_id %in% phenotypes)
#/gpfs/space/home/andreasv/cage/pipeline/annots_20/qcnorm_out/txrevise_annots/normalised/txrev/qtl_group_split_norm/GEUVADIS.LCL.tsv
counts_ann = read_tsv("countMatrix_annots_20.tsv") %>%
  melt(id.vars=c("phenotype_id"), variable.name=c("sample_id"), value.name="proportion") %>%
  filter(phenotype_id %in% phenotypes)

#dataset, counts file, promoter, variant
for (info in list(
  list(bwa_results, counts_bwa, "CAGE"),
  list(txrevise25_results, counts_tx, "txrevise"),
  list(annots_results, counts_ann, "Augmented txrevise")
)) {
  dataset = info[[1]]
  counts = info[[2]]
  title = info[[3]]
  
  dataset = filter(dataset, gene == "ENSG00000151694")
  this_promoter = dataset$top_phenotype[1]
  print(dataset$p_adjusted_beta_adjusted)
  ...
  p = ggplot(to_plot, aes(x=alleles, y=proportion, group=as.factor(alleles))) +
        ...
        labs(x="Aternative allele count", y="Normalized promoter proportions", title=title)
  print(p)
}
```
