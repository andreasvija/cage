...
length(intersect(bwa_results$gene, txrevise_results$gene)) # common total: 15 275
length(intersect(bwa_results$gene, annots_results$gene)) # same

sum(txrevise_results$total_phenotypes) # 82 835 (n=10 91 443)
sum(annots_results$total_phenotypes) # same as first
sum(annots_results$total_phenotypes) - sum(txrevise_results$total_phenotypes) # 0 (n=10 -8608)

dim(bwa_tops)[1] # BWA tops: 949 (801 with 1Mbp)
dim(tx_tops)[1] # txrevise tops: 878 (n=10 937)
dim(ann_tops)[1] # annots (N=10) tops: 941 (n=10 )

length(intersect(bwa_tops$top_variant, tx_tops$top_variant)) # 41  (n=10 44)
length(intersect(bwa_tops$top_variant, ann_tops$top_variant)) # 39
```
...
cage_over_tx = bwa_tops[! bwa_tops$gene %in% tx_tops$gene,] %>% # 669
  select(gene, top_variant)
#write_tsv(cage_over_tx, "cage_over_tx.tsv")

tx_over_cage = tx_tops[! tx_tops$gene %in% bwa_tops$gene,] %>% # 598
  select(gene, top_variant)
...
for (N in Ns) {
  ...
  removed_genes = c(removed_genes, length(setdiff(tx_tops$gene, trial_tops$gene)))
