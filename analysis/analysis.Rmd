...
#txrevise10_results = read_delim("txrev-10.permuted.txt", delim="\t", col_types=col_types, col_names=col_names, skip=1)
...
sum(annots_results$total_phenotypes) # 132 555
#sum(annots_results$total_phenotypes) - sum(txrevise10_results$total_phenotypes) # 0
sum(annots_results$total_phenotypes) - sum(txrevise25_results$total_phenotypes) # 57 819

dim(bwa_tops)[1] # BWA tops: 949
#dim(tx10_tops)[1] # txrevise10 tops: 937
dim(tx25_tops)[1] # txrevise25 tops: 866
dim(ann_tops)[1] # annots10 tops: 1150

#length(intersect(bwa_tops$top_variant, tx10_tops$top_variant)) # 44
length(intersect(bwa_tops$top_variant, tx25_tops$top_variant)) # 43
length(intersect(bwa_tops$top_variant, ann_tops$top_variant)) # 48
```
...
dim(filter(counts_tops, TPM == 0, top_in == "both"))[1] / dim(filter(counts_tops, top_in == "both"))[1] # 0.01%
```
