...
bwa_results = read_delim("bwa.permuted.multi.txt", delim="\t", col_types=col_types, col_names=col_names, skip=1)
...
counts[2:length(counts)] = counts[2:length(counts)] * 1000000 / apply(counts[2:length(counts)], 2, sum)
...
dim(filter(counts, TPM == 0))[1]/dim(counts)[1]

counts_common = counts %>%
  filter(gene %in% common_genes)
dim(filter(counts_common, TPM == 0))[1]/dim(counts_common)[1]

counts_tops = counts_common %>%
  filter(top_in != "None")
dim(filter(counts_tops, TPM == 0))[1]/dim(counts_tops)[1]
```
...
dim(filter(counts_tops, TPM == 0, top_in == "cage"))[1]/dim(filter(counts_tops, top_in == "cage"))[1]
dim(filter(counts_tops, TPM == 0, top_in == "tx"))[1]/dim(filter(counts_tops, top_in == "tx"))[1]
dim(filter(counts_tops, TPM == 0, top_in == "both"))[1]/dim(filter(counts_tops, top_in == "both"))[1]

ggplot(counts_tops, aes(TPM, fill=top_in, colour=top_in)) + 
  geom_density(alpha=0.1) + 
  scale_x_log10()
```
...
length(enough_genes)/length(common_genes)
```
