...
sum(txrevise_results$total_phenotypes) # 82 835
sum(annots_results$total_phenotypes) # same
sum(annots_results$total_phenotypes) - sum(txrevise_results$total_phenotypes) # 0

dim(bwa_tops)[1] # BWA tops: 949 (801 with 1Mbp)
dim(tx_tops)[1] # txrevise tops: 878
dim(ann_tops)[1] # annots tops: X

removed = setdiff(tx_tops$gene, ann_tops$gene)
length(removed) # annotations removed from the top: 63
added = setdiff(ann_tops$gene, tx_tops$gene)
length(added) # annotations added to the top: 45

length(intersect(bwa_tops$gene, tx_tops$gene)) # common both tops: 280
length(intersect(bwa_tops$gene, ann_tops$gene)) # common after tops: 270 (-10)
