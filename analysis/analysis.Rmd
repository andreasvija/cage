...
```{r}
top_variants = rbind(bwa_tops[c("top_variant")], tx_tops[c("top_variant")]) %>%
  unique()
write_delim(top_variants, "variants.txt")

to_visualize = bwa_tops[! bwa_tops$gene %in% tx_tops$gene,] %>% #896
  select(gene, top_variant)
write_tsv(to_visualize, "bwa_better.tsv")

to_visualize = tx_tops[! tx_tops$gene %in% bwa_tops$gene,] %>% #828
  select(gene, top_variant)
