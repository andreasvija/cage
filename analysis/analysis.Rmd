...
txrevise10_results = read_delim("txrev.permuted.txt", delim="\t", col_types=col_types, col_names=col_names, skip=1)
txrevise25_results = read_delim("txrev_25.permuted.txt", delim="\t", col_types=col_types, col_names=col_names, skip=1)
annots_results = read_delim("txrev_annots_10.permuted.txt", delim="\t", col_types=col_types, col_names=col_names, skip=1)
...
txrevise10_results = txrevise10_results %>%
...
txrevise10_results_common = txrevise10_results[txrevise10_results$gene %in% common_genes,]
...
txrevise10_results_common$p_adjusted_beta_adjusted = p.adjust(txrevise10_results_common$p_adjusted_beta, method="fdr")
...
txrevise10_results$p_adjusted_beta_adjusted = p.adjust(txrevise10_results$p_adjusted_beta, method="fdr")
...
txrevise10_results = fill_genes(txrevise10_results)
txrevise10_results_common = fill_genes(txrevise10_results_common, common=TRUE)
...
ggplot(mapping=aes(x=txrevise10_results$p_adjusted_beta_adjusted)) +
  geom_histogram()
sum(txrevise10_results$p_adjusted_beta_adjusted < 0.05)

ggplot(mapping=aes(x=txrevise25_results$p_adjusted_beta_adjusted)) +
  geom_histogram()
sum(txrevise25_results$p_adjusted_beta_adjusted < 0.05)
...
tx10_tops = txrevise10_results_common %>%
  filter(p_adjusted_beta_adjusted < 0.05) %>%
  arrange(p_adjusted_beta_adjusted)
...
dim(txrevise10_results)[1] # txrevise total: 20 176
dim(txrevise25_results)[1] # same
dim(annots_results)[1] # same
length(intersect(bwa_results$gene, txrevise10_results$gene)) # common total: 15 275
length(intersect(bwa_results$gene, txrevise25_results$gene)) # same
length(intersect(bwa_results$gene, annots_results$gene)) # same

sum(txrevise10_results$total_phenotypes) # 83 344
sum(txrevise25_results$total_phenotypes) # 74 736
sum(annots_results$total_phenotypes) # 83 344
sum(annots_results$total_phenotypes) - sum(txrevise10_results$total_phenotypes) # 0
sum(annots_results$total_phenotypes) - sum(txrevise25_results$total_phenotypes) # -8608

dim(bwa_tops)[1] # BWA tops: 949 (801 with 1Mbp)
dim(tx10_tops)[1] # txrevise10 tops: 937
dim(tx25_tops)[1] # txrevise25 tops: 878
dim(ann_tops)[1] # (N=10) annots tops: 941

length(intersect(bwa_tops$top_variant, tx10_tops$top_variant)) # 44
length(intersect(bwa_tops$top_variant, tx25_tops$top_variant)) # 41
length(intersect(bwa_tops$top_variant, ann_tops$top_variant)) # 42
```
...
tx_over_cage = tx25_tops[! tx25_tops$gene %in% bwa_tops$gene,] %>% # 598
  select(gene, top_variant)
...
ann_over_cage = ann_tops[ann_tops$gene %in% interesting$gene,] %>% # 39
  select(gene, top_variant)
...
```{r}
counts = read_tsv("../pipeline/txrevise/results/featureCounts/merged_gene_counts.txt") %>% # 25
...
common_genes_prev = common_genes
...
for (N in Ns) {
  ...
  net_change = c(net_change, dim(trial_tops)[1] - dim(compare_tx_tops)[1])
...
comparison = merge(select(txrevise10_results, gene, p_adjusted_beta_adjusted),
                   select(annots_results, gene, p_adjusted_beta_adjusted),
                   by="gene")
colnames(comparison) = c("gene", "tx10_p", "ann_p")
comparison$grouping = "N/A"
comparison = arrange(comparison, grouping)

fig4 = ggplot(comparison, aes(x=-log10(tx10_p), y=-log10(ann_p), color=grouping)) +
  labs(x="txrevise 10 -log10(p)", y="txrevise 25 + cage 10 -log10(p)", color="Olulisus") +
  geom_smooth(aes(x=-log10(tx10_p), y=-log10(ann_p)), inherit.aes=FALSE, method="lm")


comparison = merge(select(txrevise10_results, gene, p_adjusted_beta_adjusted),
                   select(txrevise25_results, gene, p_adjusted_beta_adjusted),
                   by="gene")
colnames(comparison) = c("gene", "tx10_p", "tx25_p")
comparison$grouping = "N/A"
comparison = arrange(comparison, grouping)

fig5 = ggplot(comparison, aes(x=-log10(tx10_p), y=-log10(tx25_p), color=grouping)) +
  labs(x="txrevise 10 -log10(p)", y="txrevise 25 -log10(p)", color="Olulisus") +
  geom_smooth(aes(x=-log10(tx10_p), y=-log10(tx25_p)), inherit.aes=FALSE, method="lm")




for (fig in list(fig1, fig2, fig3, fig4, fig5)) {
  print(fig +
