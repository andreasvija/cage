...
#for every gene
for (gene in c("ENSG00000151694")) {

  promoters = promoter_annots %>%
    filter(gene_name == gene)
  chromosome = promoters$chr[1]
  ...
  filter_start = min(spec$peak_start) - 10000
  filter_end = max(spec$peak_end) + 10000
  ...
  print(plotTranscripts(exons=light_blue, cdss=dark_blue, rescale_introns=FALSE))
}
