...
assigned_total / total #  46.0%
unmapped_total / total #  0%
nofeatures_total / total #54.0%

assigned_fracs = assigned / (assigned + unmapped + nofeatures)
mean(assigned_fracs) # 45.8%
sd(assigned_fracs) # 6.2%
```
...
lower_threshold = 0
commonGenes = (filter(geneCounts, geneCount > lower_threshold))$gene

length(unique(data$gene)) # 20 193
length(unique(data$promoter)) # 93 554
dim(data)
data = filter(data, gene %in% commonGenes) 
length(unique(data$gene)) # 18 546
length(unique(data$promoter)) # 90 003
dim(data)
